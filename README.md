...
啟動前需從下拉選單選擇本機可用 IPv4 位址，Flask 只會綁定所選 IP。
啟動成功後，GUI 的「網頁網址」欄位會明確顯示其他主機可連入的完整網址，例如 `http://210.70.250.152:5000`。
上次成功選擇的綁定 IP 會記錄在 `server/config.txt`；下次開啟時若該 IP 仍可用，會自動設為預設選項。
...
1. 選擇場域。
2. 輸入後端伺服器 IP。
3. 按下「伺服器連線」，等待畫面顯示連線成功。
4. 連線成功後「伺服器連線」會停用，「網頁連線」才會啟用。
5. 按下「網頁連線」，Chrome 會開啟本機代理頁面 `http://127.0.0.1:5055/`。

若啟動時無法連線到 `client/config.txt` 內的預設伺服器 IP，client GUI 會詢問是否重新輸入伺服器 IP。
