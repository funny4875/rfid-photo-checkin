...
1. 從「綁定 IP」下拉選單選擇本機可用 IPv4。
2. 按下「啟動」開始提供網頁服務。
3. 按下「停止」關閉網頁服務；停止後可重新選擇 IP。

啟動後請查看 GUI 的「網頁網址」欄位，該欄會顯示可供 client 或其他主機連入的完整網址。
上次選擇的綁定 IP 會記錄在 `server\config.txt`，下次開啟 server GUI 時會優先選取。
...
若無法連線到已儲存的預設伺服器 IP，程式會詢問是否重新輸入伺服器 IP。
...
同一後台區塊也可上傳全校照片 ZIP。照片檔名需為 `<學號>.jpg`，例如 `410054.jpg`。若勾選「上傳新照片後刪除舊照片」，系統會先清空 `server\ccsh_data`，再匯入 ZIP 內的新照片。
